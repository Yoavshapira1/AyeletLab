...
class TouchEvent:
    ...
    def get_density(self):
        ...
        if len(self.group) > 1:
            sum = 0.0
            for g in range(len(self.group) - 1):
                sum += self.group[g+1].distance(self.group[g])
            return min(sum / 2000, 1.0)
        else:
            return 0.0
...
class UDPclient:
    ...
    def broadcast(self, data):
        self.client.send_message("/channel1", data)
...
class TouchInput(Widget):
    ...
    def on_touch_up(self, touch):
        if touch.device != self.touch_mode:
            return
        for ch in self.channels:
            # Case: The touch up related to an active channel
