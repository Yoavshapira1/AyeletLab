...
class TouchEvent:
    ...
    def activate(self):
        ...
        self.prev_pos = self.cur_pos.copy()
        self.start_pos[0], self.start_pos[1] = self.cur_pos[0], self.cur_pos[1]
        self.velocity = 0.0
    ...
    def move(self, pos, dx, dy):
        ...
        if time.time() - self.prev_pos_time > self.dt:
            ...
            if ds > self.reduce_time_threshold:
                self.touch_time *= 0.1
            self.prev_pos = self.cur_pos.copy()
            self.velocity = min(1.0, np.linalg.norm([dx, dy]) * 7)
            self.prev_pos_time = time.time()
            self.touch_time += self.dt
        self.cur_pos[0] = pos[0]
        self.cur_pos[1] = pos[1]
    ...
    def get_area(self):
        ...
        if len(self.group) > 0:
            most_distant = 0
            for g in self.group:
                area = np.linalg.norm(self.cur_pos - g.spos)
    ...
    def get_qualitiative_data(self):
        ...
        if not self.switch:
            return start_pos + [0.0] * 4 + [self.mode] + [0.0]
        ...
        for ch in self.channels:
            ...
            if touch.id == ch.get_id():
                ...
                if len(ch.get_group()) > 0:
                    ...
                    ch.move([new_touch.sx, new_touch.sy], touch.dsx, touch.dsy)
